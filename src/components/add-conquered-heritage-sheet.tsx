...
interface AddConqueredHeritageSheetProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function AddConqueredHeritageSheet({
  open,
  onOpenChange,
}: AddConqueredHeritageSheetProps) {
  ...
  async function handleCreateConqueredHeritage() {
    try {
      if (!selectedEmoji) {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="border-zinc-700 bg-zinc-800 text-white">
        ...
        <div className="mt-6 space-y-6">
          ...
          <button
            onClick={handleCreateConqueredHeritage}
